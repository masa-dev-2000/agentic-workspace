...
2. ルールの手順に従い、テンプレートから実物（incident記録・依存台帳）を作成・更新する。
   - **ミス・事故の報告を受けたら、まず等級を判定する**（RULEBOOK R4）。**外に出ていない＝軽微／出たが取り返せた＝中／外に影響が残った・取り返せない＝重大。迷ったら上の等級で扱う。**
     - **軽微・中**：`incidents/near-miss-ledger.md` に**1行**追記して終わり。止血・一報は該当なしと判定するだけで調査しない。記録ファイルは作らない。**同じ構造が3件たまったら重大へ格上げ**（台帳末尾の件数表を必ず確認する）。ルール・手順の変更に至ったものは記録ファイルへ昇格させ台帳からリンクする
