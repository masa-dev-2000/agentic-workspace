...
46 skills are wired into every runtime. Verify with `python -X utf8 scripts/list_skill_sources.py --names`.
<!-- /generated:skill-count -->
...
The block above is regenerated by `scripts/sync_generated_docs.py` and a stale
copy fails `validate_workspace.py`, so it cannot drift from `config/wiring.json` — if it looks wrong, the JSON file is wrong, not this doc. For the field
